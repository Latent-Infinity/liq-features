...
def test_feature_store_caches_results(tmp_path: Path) -> None:
    ...
    feature_def = FeatureDefinition(
        name="test_feature", func=simple_feature, lookback=0, input_column="close"
    )
...
def test_feature_store_stores_feature_set_with_timeframe(tmp_path: Path) -> None:
    ...
    fs.compute_feature_set(
        df, feature_set, symbol="EUR_USD", timeframe="1m", force_recalculate=True
    )
...
def test_feature_store_resolves_dependencies(tmp_path: Path) -> None:
    ...
    def base(df: pl.DataFrame, _input_col: str) -> pl.DataFrame:
        return df.select("timestamp", pl.col("close").alias("value"))

    def dependent(df: pl.DataFrame, _input_col: str) -> pl.DataFrame:
    ...
    dep_def = FeatureDefinition(
        name="dep", func=dependent, lookback=0, input_column="close", dependencies=["base"]
    )
    ...
    outputs = fs.compute_feature_set(
        df, fs_def, symbol="EUR_USD", timeframe="1m", force_recalculate=True
    )
